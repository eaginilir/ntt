...
struct __uint128_t {
    ...
    __host__ __device__ uint64_t mod(uint64_t mod) const {
        // 2^64 % mod = (~0ULL % mod) + 1
        uint64_t base = ((~(uint64_t)0) % mod) + 1;
        uint64_t rem = (hi % mod) * base % mod;
        rem = (rem + (lo % mod)) % mod;
        return rem;
    }
...
struct Barrett {
    ...
    __host__ __device__ inline uint64_t reduce(uint64_t x) const {
        // q = floor(x * factor / 2^64)
        __uint128_t q = __uint128_t::mul(x, factor);
        uint64_t q1 = q.hi;
        uint64_t r = x - q1 * mod;
        if (r >= mod) r -= mod;
        return r;
    ...
    // 128位乘法后直接取模
    __host__ __device__ inline uint64_t reduce128(const __uint128_t& x) const {
        // x.hi < mod 保证安全
        return x.mod(mod);
    }
    __host__ __device__ inline uint64_t mul(uint64_t a, uint64_t b) const {
        return reduce128(__uint128_t::mul(a, b));
    }
